...
void main() {
    ...
    //float info1=texture2D(u_tex1,st).g;
    //vec3 col=vec3(texh(uv*5.0, info+0.3-breathing*0.3));
    //vec3 col=vec3(texh(uv*5.0, info1+0.3-breathing*0.3));

    vec4 tex0 = texture2D(u_tex0, uv); //new
    vec4 tex1 = texture2D(u_tex1, uv); //new
    vec3 col = vec3(texh(uv*5.0, info+0.3-breathing*0.3)); 
    col = mix(col, tex1.rgb, 0.2); 
