...
float mouseEffect(vec2 uv, vec2 mouse, float size)
{
    ...
    return smoothstep(size, size+0.3*(breathing+0.5), dist);  //size
    return smoothstep(size, size+0.3*(breathing+0.5), dist);  //size
}
...
void main()
{
    ...
    //vec2 mouse = vec2(0.6, 0.75-1.2*sin(2.0*3.14159));
    //vec2 mouse = vec2(0.6, 0.3*(breathing+0.5));
    vec2 mouse = vec2(0.6,0.3+0.7*sin(uv.y));
    ...
    vec4 c;
     ...
     vec4 inkColor = vec4(1.0, 1.0,0.5+breathing , 0.8);;
